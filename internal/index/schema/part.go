...
func SelectPackagesByParts(ctx context.Context, db Querier, parts []string, pkgs []ModulePackage) ([]ModulePackage, error) {
	const query = `
WITH RECURSIVE 
        matches(remaining_path, part_id, path_depth, part_length) AS (
                VALUES (? || '/', NULL, 0, 0)
                UNION
                SELECT 
                ...
                FROM matches, part
                WHERE
                        name LIKE substr(remaining_path, 1, instr(remaining_path, '/')-1) || '%' AND
                        (part.parent_id = matches.part_id OR matches.part_id IS NULL)
                        AND remaining_path != ''
                ORDER BY 
                        3 DESC, 
                        4 ASC
...
SELECT 
        ...
        package_import_path,
        dir
FROM matches, part_package USING(part_id), module_package USING (package_id) 
WHERE 
        remaining_path = ''
ORDER BY 
